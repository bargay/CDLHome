...
function populateCountries(countryElementId){
	...
	for (var i = 0; i < country_arr.length; i++){
		var country_code = country_arr[i];
		country_arr[i] = {
			name: country_code[0],
			dialCode: country_code[1]
		};
		(countryElement.options[countryElement.length] = new Option(country_arr[i].name, country_arr[i].name)).setAttribute('data-dialcode', country_arr[i].dialCode);
	}
}

if ($('#form_selectCountry').size() > 0){
	populateCountries('form_selectCountry');
	
	// Set Singapore as default country
	$('#form_selectCountry option[value="Singapore"]').attr('selected', 'selected');

	var selectedCountryDial = $('#form_selectCountry option[selected]').data('dialcode');
	$('.form-input-dialcode').attr('value', '+'+ selectedCountryDial);
	
	// Select dropdown on change
	$('#form_selectCountry').on('change', function(){
		$('.form-input-dialcode').attr('value', '+'+ $('#form_selectCountry option').filter(':selected').data('dialcode'));
	});
}
...
if ($('input[name="email"]').size() > 0){
	$('input[name="email"]').on('blur keypress', function(){
		var email = $(this).val();
		var re = /[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}/igm;
		if (re.test(email)){
			$('.error-msg.email-is-invalid').hide();
		} else {
			$('.error-msg.email-is-invalid').show();
		}
	});
}
