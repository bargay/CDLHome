...
document.createElement("picture"), $(window).load(function() {
    ...
    if ($("#sequence").size() > 0) {
        ...
        if(slideNum == '1') {
          $(".seq-canvas > li").addClass('one-banner-seq');
        }
    }
    ...
    $(".select-jump").on("change", function() {
        ...
        stickyNav()
    }, 1200), loadMatchHeight(), ellipsis(), setBgImg(), filterPropShow(), leaseFilter(), highlightTabs(), selectPropSite(), getUpdatedForm(), contactCountry(), ShowMoreMobile(), stickyMenu(), commaNumbers(), PointerEventsPolyfill.initialize()
