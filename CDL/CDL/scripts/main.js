function stickyMenu() {
    $('#nav-sticky .new-sticky-wrp .dropdown-menu > li > a').click(function (e) {
        // if(!isMobile()) {
        // e.preventDefault();
    ...
    if ($('#nav-sticky .nav-sticky-wrp .dropdown-menu > li').length > 4) {
        $('#nav-sticky .nav-sticky-wrp').addClass('moreFour-wrp');
        $('#nav-sticky .nav-sticky-wrp .dropdown-menu').addClass('moreFour');
        $('.nav-sticky-left .visible-xs-inline-block').addClass('moreFour');
    }
}

function ShowMoreMobile() {
    $('.ShowMoreBtn').click(function () {
        $('.mobileShowMore').fadeIn();
        $('.ShowMoreBtn > .btn.btn-primary').css('display', 'none');
    });
}
...
function loadMatchHeight() {
    ...
    if (isMobile()) {
        $('.match-height-2').addClass("match-height");
    }
}
...
function highlightTabs() {
    ...
    $(window).scroll(function () {
        var scrollPosition = $(window).scrollTop() + viewportHeight,
            footerOffset = $("#footer").offset().top - 10;
...
document.createElement("picture"), $(window).load(function () {
    ...
    $(".select-jump").on("change", function () {
        ...
        stickyNav()
    }, 1200), loadMatchHeight(), ellipsis(), setBgImg(), filterPropShow(), leaseFilter(), highlightTabs(), selectPropSite(), getUpdatedForm(), contactCountry(), ShowMoreMobile(), stickyMenu(), PointerEventsPolyfill.initialize()
...
$(document).ready(function () {
    ...
    if ((hash && hash != '#') && isElementExist('.tabs-content')) {
        hash = hash.substr(1); /* remove the "#" */
        $('.tabs-content .nav-tabs li a').filter(function () {
            return $(this).attr('href').indexOf(hash) > -1;
        }).click();/* click the matching tab */
    }

    $('.buyer-guide-tabs a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
        var target = $(e.target).attr("href") // activated tab

        if ("#tab_2" == target) {
            //small medium
            var heightArray = $(".row.wb-highlights.wb-s .wb-entry").map(function () {
                return $(this).height();
            }).get();
            var maxHeight = Math.max.apply(Math, heightArray);
            $(".row.wb-highlights.wb-s .wb-entry").height(maxHeight);

            // medium height
            var heightArray = $(".row.wb-highlights.wb-m .wb-entry").map(function () {
                return $(this).height();
            }).get();
            var maxHeight = Math.max.apply(Math, heightArray);
            $(".row.wb-highlights.wb-m .wb-entry").height(maxHeight);
        }
    });
